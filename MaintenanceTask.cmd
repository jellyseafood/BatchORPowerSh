...
:Intro
	...
	echo Check source code to verify targeted tasks prior to running
	echo/
	...
	echo    C - Check Settings via excel(TaskList.csv, Uppercase C)
	echo    N - Check Settings via notepad (TaskList.csv, Uppercase N)
...
:PromptUserAction
	choice /C 12345CNSx /N /CS /M "Pick one [1,2,3,4,5,C,N,S,x]: "
	...
	goto myPick_%myChoice%
		...
		:myPick_6
			echo/
			echo You chose Check Settings (TaskList.csv, Uppercase C)
			echo/
		...
		:myPick_7
			echo/
			echo You chose Check Settings via notepad (TaskList.csv, Uppercase N)
			echo/
			echo The arguments set here are CRITICAL                                   
			echo *disabling or enabling unintended tasks might be harmful to the system
			echo Each line consists of 3 arguments (order of each argument is important)
...
:LogFileSetup
	...
	set "logFile=%logDir%\%storedPick%_%myDate%_%myTime%.log"
...
REM E for excel, C for view credential
REM add L view log folder
REM echo all important directories, including the script itself during startup
